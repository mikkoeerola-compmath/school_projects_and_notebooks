...
%
% Client process (tests simple server behaviour). new_client initiates a client, other functions perform actions
% towards servers.
%
% Requests to Servers follow the structure from material: {{ClientPid,Ref},Request}

-module(client1).
-export([new_client/1, client1_test/0, subscribe/2, write/3, read/2]).

% Init client with name
new_client(Name) ->
    PID = spawn(?MODULE, client1_test, []),
    register(Name, PID),
...
client1_test() ->
    case whereis(server1) of
        undefined ->
            io:format("server1 not found.~n"),
            exit(server_not_found);
        Server1 ->
            write(Server1, key1, value1),
            timer:sleep(10),
            write(Server1, key2, value2),
            timer:sleep(10),
            subscribe(Server1, key1),
            timer:sleep(10),
            read(Server1, key2),
            rpc_loop()
    end.
