...
wikitextsRDD: RDD[str] = spark.sparkContext.textFile(path).filter(lambda line: line.strip() != "")
...
wordsRDD: RDD[str] = wikitextsRDD \
    ...
    .filter(lambda w: re.search(r'^[0-9]', w) is None)
...
filtered = wordCountRDD \
    .filter(lambda w: len(w[0]) == 8 and w[0].startswith('s')) \
    .filter(lambda word: word[0] != 'software') \
    .collectAsMap() 

askedWord, wordCount = sorted(filtered.items(), reverse=True)[0]
