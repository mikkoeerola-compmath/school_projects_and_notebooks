...
from pyspark.sql.types import DoubleType, StructField, StructType, StringType, DateType, IntegerType
...
trainingDF, testDF = dataDF.randomSplit([.8, .2], 42)
...
testError: float = regressionEvaluator.evaluate(testPredictions)
...
file = 'abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex5/static/superstore_sales.csv'
staticSalesDF: DataFrame = spark.read  \
  .option("header", "true") \
  .option("delimiter", ";") \
...
staticBestDaysDF: DataFrame = staticSalesDF \
  ...
  .agg(F.round(F.sum(F.col('productPrice')*F.col('productCount')),1).alias('Total Sales')) \
...
myStreamingIdentifier: str = "ckmiee"  # for example: "my_very_unique_identifier" or "firstname_lastname"
...
streamingBestDaysDF: DataFrame = streamingSalesDF \
  .groupBy(F.col('orderDate')) \
  .agg(F.round(F.sum(F.col('productPrice')*F.col('productCount')),1).alias('Total Sales'), F.sum(F.col('productCount'))) \
  .orderBy(F.desc('Total Sales'))
...
# MAGIC %md
# MAGIC - I used ChatGPT to build schemas and also debug some code parts. It was helpful and didn't have to manually write the schema.
