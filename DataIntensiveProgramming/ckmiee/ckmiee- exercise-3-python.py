...
file_weather = 'abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex3/weather/measurements/daily_weather.parquet'
file_city = 'abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex3/weather/metadata/cities.csv'
file_country = 'abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex3/weather/metadata/countries.csv'

weatherDF: DataFrame = spark.read.parquet(file_weather)

cityDF: DataFrame = spark.read \
    .option("header", "true") \
    .option("sep", ",") \
    .option("inferSchema", "true") \
    .csv(file_city)

countryDF: DataFrame = spark.read \
    .option("header", "true") \
    .option("sep", ",") \
    .option("inferSchema", "true") \
    .csv(file_country)
...
metadataDF: DataFrame = cityDF.join(countryDF, cityDF.iso3 == countryDF.iso3).select('station_id', 'city_name', 'latitude', 'country', 'capital')

fullDF: DataFrame = weatherDF.join(metadataDF, weatherDF.station_id == metadataDF.station_id)
...
w = Window.partitionBy('country').orderBy('min_temp_c')

coldDayDF = fullDF \
    .withColumn('rank', F.row_number().over(w)) \
    .filter(F.col('rank') == 1) \
    .select('country', 'min_temp_c', 'date') \
    .orderBy('min_temp_c')

hotDayDF: DataFrame = fullDF \
    .withColumn('rank', F.row_number().over(w)) \
    .filter(F.col('rank') == 1) \
    .select('country', 'max_temp_c', 'date') \
    .orderBy('max_temp_c')
...
# MAGIC %md
# MAGIC 1. Spark is a processing framework to distribute and scale computing horizontally for multiple nodes. This scaling can be done basically infinitely so even heavy data progressing can be done quickly. It is considered as a baseline for data engineering so it is used in this class. 
# MAGIC 2. When the data is really big, spark can be a lot faster. Also freeing memory and processing power for other actions.
# MAGIC 3. When the data or the application reguires a lot of suffeling from spark. Internet usage.
...
# MAGIC %md
# MAGIC I used AI to in task 3 with understanding window functions. I worked alone.
