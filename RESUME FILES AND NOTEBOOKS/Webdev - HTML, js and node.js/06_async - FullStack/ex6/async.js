...
function drawArrows(actors, timeout, drawArrow, i = 0) {
    for (i = 0; i < 2*actors.length-1; ++i) {
        (async () => {
            await waitToDraw(actors, timeout, drawArrow, i);
        });
    }
};

async function waitToDraw(actors, timeout, drawArrow, i) {
    setTimeout(drawArrow(i, actors.length), timeout);
}
