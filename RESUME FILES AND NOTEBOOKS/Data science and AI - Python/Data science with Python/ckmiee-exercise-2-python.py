...
myData = [("point1", 100, 3), ("point2", 200, 1), ("point3", 300, 5)]
dfMyData = spark.createDataFrame(myData).toDF("pointId", "value", "measure quility")
display(dfMyData)
...
file = 'abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex2/numbers/numbers.csv'
...
file = 'abfss://shared@tunics320f2025gen2.dfs.core.windows.net/exercises/ex2/weather/nordics_weather.csv'
...
minTemp: float = weatherDF.select(F.min("temperature_min")).first()[0]
maxTemp: float = weatherDF.select(F.max("temperature_max")).first()[0]
...
weatherDFWithNewColumns: DataFrame = weatherDF \
    .withColumn('measurment_year', F.year('date')) \
    .withColumn('measurment_weekday', F.dayofweek('date'))
...
yearlyTemperatureDF: DataFrame = weatherDFWithNewColumns \
    .groupBy('measurment_year') \
    .agg(F.min('temperature_min'), F.max('temperature_max')) \
    .orderBy('measurment_year', ascending=False)
...
avgFinlandDF: DataFrame = weatherDFWithNewColumns \
    .where((F.col('measurment_year') == 2017) & (F.col('country') == 'Finland')) \
    .groupBy('measurment_weekday') \
    .agg(F.round(F.avg('precipitation'), 2).alias("precipitation_avg"), F.round(F.avg('snow_depth'), 2).alias("snow_depth_avg")) \
    .orderBy('measurment_weekday', ascending=True)
...
# MAGIC %md
# MAGIC 1) It means that the algorithms and code don't need changing when you add more data or computations.
# MAGIC 2) Horizontal scaling means more adding computors (or cloud) doing the computing, vertical scaling refers to more powerfull computing. In both cases the computing power increases but in different ways.
# MAGIC 3) Faster computing, shorter latency or runtime, more data can be managed and analyzed.
# MAGIC 4) With vertical: expenses, limit of computing power, vulnerable since more relays on single computer. Horizontal: How to do parallel programming, network load, mistakes in combining results
...
# MAGIC %md
# MAGIC I used chatGPT to find out how to round floats together with aggregation. I was helpful but could have also used sparkDoc. I worked alone.
