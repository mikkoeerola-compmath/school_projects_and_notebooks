...
cubedList: list[int] = map(lambda x: x ** 3, myList)
cubeSum: int = reduce(lambda x, y: x + y, cubedList)

smallestFuncValue: int = reduce(lambda x, y: min(x, y), map(lambda x: x ** 2 - 34 * x + 325, myList))
...
temperatureReadings.sort(key=lambda x: x[0])
averageTemperatures: dict[str, float] = {}
for sensor, group in groupby(temperatureReadings, key=lambda x: x[0]):
    averageTemperatures[sensor] = average(map(lambda x: x[1], list(group)))
...
# MAGIC %md
# MAGIC The reduce function traverses the list (not sure if from L to R or from R to L) and feeds an element and the reduction to the lambda function. The lambda function takes the pair (the reduction) and an element and adds the element to the pairs first element and increases the counter on the second element. When the travese is ready, the reducion is all that remains and the reduce function gives us that reduction. The reduction now has the sum and count of the values as it's components. Finally the average function calculates the average using these numbers.
# MAGIC
# MAGIC It might be more efficient and dealing with infinite lists (big lists) not having to calculte the whole list right away might be beneficial.
# MAGIC
# MAGIC If the input list is empty he reduce function just gives the initialized accumulator so we have (0.0, 0). The averega function then tries to divide with 0 which gives an error. This might break our implementation. We should handle the possibility of an empty list.
...
def seventhRoot(x: float) -> float:
    ...
    new: int = 1
    while flag:
        new = 1/7*(6*seventhRoot+x/(seventhRoot**6))
        if abs(new-seventhRoot) < 1e-6:
            flag = False
        else:
            seventhRoot = new
    return seventhRoot
...
# MAGIC %md
# MAGIC
# MAGIC 1. I have about 60 credits worth of computer science studies most of which include programming. I have been programming more seriously for 3 years. My python skills are little dusty and I aim to develop both in Scala and in Python. I have done two courses worth of functional programming and I find it well suitable for me. I haven't combined it with Python so it was nice too see it's quite similar than in javascript.
# MAGIC 2. I really liked Databricks. Easy to use and simple workflow. The clone feature is nice with the excercises
# MAGIC 3. They were pretty easy. I had to search for python library for some functions and definitions such as groupby function.
# MAGIC
...
# MAGIC %md
# MAGIC I did some debugging with chatGPT. It did help to remember Python's syntax.
# MAGIC I worked alone.
