...
def recommend_movies(user_id, user_similarity, user_movie_matrix):
    ...
    # 1. Sort all users by similarity (exclude target user) (and take 200 closest for speed)
    neighbors = user_similarity[target_user].drop(target_user).sort_values(ascending=False)[0:199]

    # 2. Keep only neighbors with at least 5 commonly rated movies
    target_non_null = target_ratings.dropna().index
    valid_neighbors = [
        user for user in neighbors.index
        if len(target_non_null.intersection(user_movie_matrix.loc[user].dropna().index)) >= 5
    ]
    neighbors = neighbors.loc[valid_neighbors]

    # 3. Movies the target user has not rated
    unrated_movies = target_ratings[target_ratings.isna()].index

    avg_target = target_ratings.mean()
    predicted = {}

    # 4. Compute predictions
    for movie in unrated_movies:

        movie_ratings = user_movie_matrix[movie].dropna()
        rated_users = movie_ratings.index.intersection(neighbors.index)

        # Only consider top-100 similar users
        top_users = neighbors.loc[rated_users].head(100)

        if top_users.empty:
            continue

        # User-average centering
        user_avgs = user_movie_matrix.loc[top_users.index].mean(axis=1)
        deviations = movie_ratings.loc[top_users.index] - user_avgs
        ...
        predicted[movie] = avg_target + (weights * deviations.values).sum() / denom

    # Convert movie IDs → titles
    pred_series = pd.Series(predicted)
    #pred_series.index = pred_series.index.map(movie_id_to_title)
...
def group_recommendations(user_ids: list, um_matrix):
    all_recommendations = {}

    for user in user_ids:
        recs = (recommend_movies(user, user_similarity, um_matrix))
        recs = recs[~recs.index.duplicated(keep='first')]
        all_recommendations[user] = recs
    return pd.DataFrame(all_recommendations).dropna()

def aggregate_group_mean(group_df):
    return group_df \
        .transpose() \
        .agg('mean') \
